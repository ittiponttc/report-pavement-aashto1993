...
def main():
    ...
    with tab1:
        ...
        # วัสดุกำหนดเอง (Custom Materials)
        st.subheader("✏️ วัสดุกำหนดเอง")
        st.info("💡 เพิ่มวัสดุใหม่ที่ไม่มีในรายการ — จะปรากฏใน dropdown ของ Tab โครงสร้างชั้นทาง")

        if 'custom_surface_materials' not in st.session_state['price_library']:
            st.session_state['price_library']['custom_surface_materials'] = {}
        if 'custom_base_materials' not in st.session_state['price_library']:
            st.session_state['price_library']['custom_base_materials'] = {}

        # เพิ่มวัสดุผิวทาง (บาท/ตร.ม.)
        st.markdown("**เพิ่มวัสดุผิวทาง (บาท/ตร.ม.)**")
        cust_surf = st.session_state['price_library']['custom_surface_materials']
        cs_col1, cs_col2, cs_col3 = st.columns([3, 1.5, 1])
        with cs_col1:
            new_surf_name = st.text_input("ชื่อวัสดุผิวทาง", value="", key="new_surf_name", placeholder="เช่น Polymer Modified Asphalt")
        with cs_col2:
            new_surf_price = st.number_input("ราคา (บาท/ตร.ม.)", value=300.0, min_value=0.0, step=10.0, key="new_surf_price")
        with cs_col3:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("➕ เพิ่ม", key="add_cust_surf"):
                if new_surf_name.strip():
                    cust_surf[new_surf_name.strip()] = new_surf_price
                    st.success(f"✅ เพิ่ม {new_surf_name.strip()}")
                    st.rerun()

        if cust_surf:
            for cname in list(cust_surf.keys()):
                cc1, cc2, cc3 = st.columns([3, 1.5, 1])
                with cc1:
                    st.text(cname)
                with cc2:
                    new_p = st.number_input("ราคา", value=float(cust_surf[cname]), key=f"cedit_s_{cname}", step=10.0, label_visibility="collapsed")
                    cust_surf[cname] = new_p
                with cc3:
                    if st.button("🗑️", key=f"cdel_s_{cname}"):
                        del cust_surf[cname]
                        st.rerun()

        st.markdown("---")

        # เพิ่มวัสดุพื้นทาง (บาท/ลบ.ม.)
        st.markdown("**เพิ่มวัสดุพื้นทาง/รองพื้นทาง (บาท/ลบ.ม.)**")
        cust_base = st.session_state['price_library']['custom_base_materials']
        cb_col1, cb_col2, cb_col3 = st.columns([3, 1.5, 1])
        with cb_col1:
            new_base_name = st.text_input("ชื่อวัสดุพื้นทาง", value="", key="new_base_name", placeholder="เช่น Recycled Concrete Aggregate")
        with cb_col2:
            new_base_price = st.number_input("ราคา (บาท/ลบ.ม.)", value=400.0, min_value=0.0, step=10.0, key="new_base_price")
        with cb_col3:
            st.markdown("<br>", unsafe_allow_html=True)
            if st.button("➕ เพิ่ม", key="add_cust_base"):
                if new_base_name.strip():
                    cust_base[new_base_name.strip()] = new_base_price
                    st.success(f"✅ เพิ่ม {new_base_name.strip()}")
                    st.rerun()

        if cust_base:
            for cname in list(cust_base.keys()):
                cc1, cc2, cc3 = st.columns([3, 1.5, 1])
                with cc1:
                    st.text(cname)
                with cc2:
                    new_p = st.number_input("ราคา", value=float(cust_base[cname]), key=f"cedit_b_{cname}", step=10.0, label_visibility="collapsed")
                    cust_base[cname] = new_p
                with cc3:
                    if st.button("🗑️", key=f"cdel_b_{cname}"):
                        del cust_base[cname]
                        st.rerun()
    ...
    with tab2:
        ...
        for i, ทางเลือก in enumerate(st.session_state.ทางเลือกทั้งหมด):
            with st.expander(f"{'✅' if ทางเลือก.เปิดใช้งาน else '❌'} ทางเลือก {i+1}: {ทางเลือก.ชื่อ} | {ทางเลือก.ต้นทุนก่อสร้าง:,.2f} บาท/ตร.ม.", expanded=(i == 0)):
                ...
                # ชั้นวัสดุ — ใช้ Dropdown เลือกจาก Library (เหมือน NPV-Cost v3.0)
                st.markdown("---")
                st.subheader("🧱 ชั้นวัสดุ")

                # สร้าง Library วัสดุสำหรับ dropdown
                lib = st.session_state.get('price_library', {})

                # --- ตัวเลือกวัสดุผิวทาง ---
                wearing_options = ['AC Wearing Course', 'PMA Wearing Course']
                binder_options = ['AC Binder Course']
                ac_base_options = ['AC Base Course']
                concrete_options = ['JPCP', 'JRCP', 'CRCP']
                other_surface_options = ['Tack Coat', 'Prime Coat', 'Non Woven Geotextile', 'Steel Reinforcement']
                # รวม custom surface
                custom_surf = lib.get('custom_surface_materials', {})
                all_surface_names = wearing_options + binder_options + ac_base_options + list(custom_surf.keys()) + other_surface_options
                ...
                base_layers = []
                for layer in ทางเลือก.ชั้นวัสดุ:
                    if layer.get('layer_type') == 'base' or layer.get('qty_unit') == 'cu.m':
                ...
                for j, layer in enumerate(surface_layers):
                    ...
                    is_ac_base = ('base' in name_lower and 'ac' in name_lower) or ('base course' in name_lower and 'cement' not in name_lower and 'crushed' not in name_lower and 'soil' not in name_lower)
                    is_concrete = 'concrete' in name_lower or 'ksc' in name_lower

                    with cols[0]:
                        if is_wearing:
                            opts = wearing_options
                            def_idx = next((k for k, v in enumerate(opts) if v.lower() in name_lower), 0)
                            selected = st.selectbox("วัสดุ", opts, index=def_idx, key=f"sf_{i}_{j}", label_visibility="collapsed")
                        elif is_binder:
                            selected = st.selectbox("วัสดุ", binder_options, index=0, key=f"sf_{i}_{j}", label_visibility="collapsed")
                        elif is_ac_base:
                            selected = st.selectbox("วัสดุ", ac_base_options, index=0, key=f"sf_{i}_{j}", label_visibility="collapsed")
                        elif is_concrete:
                            conc_def = 0
                            for ci, ct in enumerate(concrete_options):
                                if ct.lower() in name_lower or ct.lower() in ทางเลือก.ประเภท.lower():
                                    conc_def = ci
                            ...
                            try:
                                ...
                                def_idx = 0
                            selected = st.selectbox("วัสดุ", all_surface_names, index=def_idx, key=f"sf_{i}_{j}", label_visibility="collapsed")

                    with cols[1]:
                        thick = st.number_input("หนา", value=float(layer['thickness']), min_value=0.0, step=1.0, key=f"st_{i}_{j}", label_visibility="collapsed")

                    # คำนวณราคาอัตโนมัติจาก Library
                    auto_price = layer.get('unit_cost', 0)
                    if is_wearing or is_binder or is_ac_base:
                        ac_key = selected
                        prices = lib.get('ac_prices', {}).get(ac_key, {})
                        if prices:
                            p = prices.get(thick)
                            if p is None:
                                closest = min(prices.keys(), key=lambda x: abs(x - thick))
                                p = prices.get(closest)
                            if p is not None:
                                auto_price = p
                    elif is_concrete:
                        conc_type = sel_conc if 'sel_conc' in dir() else ทางเลือก.ประเภท
                        prices = lib.get('concrete_prices', {}).get(conc_type, {})
                        if prices:
                            p = prices.get(int(thick))
                            if p is None:
                                closest = min(prices.keys(), key=lambda x: abs(x - thick))
                                p = prices.get(closest)
                            if p is not None:
                                auto_price = p
                    elif selected in custom_surf:
                ...
                for idx in sorted(del_surf, reverse=True):
                    if len(updated_surface) > 0:
                        updated_surface.pop(idx)

                # ปุ่มเพิ่มผิวทาง
                if st.button("➕ เพิ่มชั้นผิวทาง", key=f"add_surf_{i}"):
                    updated_surface.append({
                        'name': 'AC Wearing Course', 'thickness': 5.0, 'unit': 'cm',
                        'qty_unit': 'sq.m', 'unit_cost': 250.0, 'layer_type': 'surface'
                    })
                ...
                num_base = st.number_input("จำนวนชั้นพื้นทาง/รองพื้นทาง", value=max(len(base_layers), 1),
                                           min_value=1, max_value=8, key=f"num_base_{i}")
                ...
                for j in range(int(num_base)):
                    ...
                    if j < len(base_layers):
                        def_name = base_layers[j]['name']
                        def_thick = base_layers[j]['thickness']
                    else:
                        def_name = all_base_names[0]
                        def_thick = 20.0

                    try:
                        def_idx = all_base_names.index(def_name)
                    except ValueError:
                        def_idx = 0

                    with cols[0]:
                        sel_base = st.selectbox("วัสดุ", all_base_names, index=def_idx, key=f"bm_{i}_{j}", label_visibility="collapsed")
                    with cols[1]:
                ...
                # รวม layers กลับ
                ทางเลือก.ชั้นวัสดุ = updated_surface + updated_base

                # ===== คำนวณต้นทุนรวม =====
                total_cost_sqm = 0.0
                for layer in updated_surface:
                    if layer.get('unit') == 'Layer':
                        total_cost_sqm += layer['unit_cost'] * layer['thickness']
                    else:
