...
def create_word_report(
    ...
    for result in comparison_results:
        ...
        row_cells[1].text = f"{result['d_inch']:.2f}"
    ...
    summary = f"""
    ความหนาที่เลือก: {selected_d_cm:.0f} ซม. ({selected_d_inch:.2f} นิ้ว)
...
def main():
    ...
    with col1:
        ...
        d_cm_selected = st.slider(
            ...
            value=30,
            step=1,
            help="ความหนาแผ่นพื้นคอนกรีต (20-45 ซม.)"
        )
        # แปลงเป็นนิ้วสำหรับคำนวณ
        d_inch_selected = d_cm_selected / 2.54
        st.info(f"D = **{d_cm_selected} ซม.** ({d_inch_selected:.2f} นิ้ว)")
    ...
    with col2:
        ...
        # ใช้ความหนาเป็นเซนติเมตรที่ลงตัว
        ...
        if passed_selected:
            ...
            ความหนา D = {d_cm_selected} ซม. ({d_inch_selected:.2f} นิ้ว) 
